...
- [x] 2026-07-16 v1.0.9 feature commit `8f7bba9` and annotated `v1.0.9` published after ADR-080, migration 0062, bounded graph-aware initial composition and durable `skill_call` authority passed 270 unit, 59 contract, 58 integration, 46 E2E, architecture/OpenAPI/build/migrations.
...
- [x] 2026-07-16 v1.0.11 bug-fixed commit `fe3b126` and annotated `v1.0.11-bug-fixed` published after contradictory/default and malformed remote authority rejection, frozen snapshots, PostgreSQL evidence revalidation and atomic administrator override/audit passed the full gate in 85,191 ms.
- [x] 2026-07-16 v1.0.12 feature commit `01e2d44` and annotated `v1.0.12` published after ADR-083, strict durability/authority refinement, durable-only admission, generic provider/dimension-isolated pgvector storage through 0064 and queryable post-commit warnings passed the full gate in 86,193 ms.
- [x] 2026-07-16 v1.0.12 bug-fixed commit `21b9f79` and annotated `v1.0.12-bug-fixed` published after preventing model authority elevation, deterministically rejecting forged durable device state, freezing bounded Memory/Embedding snapshots and passing the full gate in 85,277 ms.
- [x] 2026-07-16 v1.0.13 feature commit `a13d8e7` and annotated `v1.0.13` published after ADR-084 process-local Task notifications, post-commit publication across every Task write, PostgreSQL-authoritative safety reload, snapshot-on-timeout and waiter release passed 296 unit, 64 contract, 60 integration and 46 E2E in 89,011 ms.
- [ ] 2026-07-16 v1.0.13 bug-fixed audit removes duplicate deadline/post-close reads, rejects execution after close and proves rollback emits no notification. Required `pnpm verify` passed in 88,363 ms, `pnpm demo:local` passed and `pnpm demo:acceptance` passed 46/46; publication remains.
...
- 2026-07-16: v1.0.11 confirmation review found that rendering the current MCP registry value would allow discovery refresh to drift from the Planner's authority. Migration 0063 therefore stores the exact Tool semantics array on each Workflow plan and attempt; repair planning cannot replace a confirmed snapshot.
- 2026-07-16: the first v1.0.11 real persistence attempt could not start because the operator-managed PostgreSQL endpoint refused connections. After the operator restored it, 0063 persistence/rollback, the complete integration/E2E/smoke suite and `pnpm verify` passed without any Docker command.
- 2026-07-16: the first v1.0.12 full gate correctly caught a new Memory Console assertion placed under the unrelated System panel test. Moving the same behavioral assertion to the Memory panel scenario restored test ownership; the targeted Console suite and the complete second gate then passed.
- 2026-07-16: v1.0.12 adversarial review found that strict shape validation did not stop a model from naming `admin` or `skill_experience` as durable authority, one positive test allowed an online observation to be labeled durable, and TypeScript readonly references could drift after an asynchronous embedding call. Application provenance/dynamic-state policy plus bounded runtime snapshots close all three gaps before persistence.
- 2026-07-16: PostgreSQL timestamps may share millisecond precision across meaningful Task transitions. Current notifier waiters must therefore wake on every publish, while a cached snapshot may wake a later waiter only when its timestamp is newer; using inequality for cached state would turn an older cache entry into another busy loop.
- 2026-07-16: raising real-E2E safety polling to 5,000 ms still passed all 46 A2A scenarios. This exposes missing production publish wiring promptly while preserving a separate 100 ms unit-only safety-recovery test.
- 2026-07-16: the feature implementation released notifier waiters on close but then performed one more PostgreSQL read and event publication; the timeout edge could likewise perform a second final read. The bug-fixed flow terminates before post-close I/O and reuses the final safety read as the timeout snapshot.
...
- 2026-07-16: v1.0.9 bug-fixed review found that shape-valid inherited/database contexts could name a disconnected child and that loading the entire relation table weakened the advertised composition bound. Domain reachability/uniqueness/depth checks plus indexed source/type/limit reads now make the bound executable at every trust boundary.
...
- 2026-07-16: accept ADR-083. Memory durability and authority are domain evidence; strict model refinement precedes automatic admission, only durable results enter long-term storage, embeddings remain provider/dimension isolated, and all post-terminal Memory work stays non-authoritative.
- 2026-07-16: accept ADR-084. The application owns a process-local Task notification port, every PostgreSQL Task mutation publishes after commit, and A2A always reloads PostgreSQL on wake. Notifications remain ephemeral optimization, never Task authority or recovery state.
