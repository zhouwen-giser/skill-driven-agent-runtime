...
- The Task plan stores the exact resolution ID. Migration 0060 enforces that the record belongs to the same Task, Goal version and selected Skill version; confirmation-time execution reads this exact record rather than a latest-by-time query.
- WorkflowControl retains the resolved value for ordinary replans. Goal Patch resolves formal Skill input against the proposed new Goal version before invalidating old execution state or creating its fresh-confirmation plan. Unresolved patched input rejects the Patch without changing Goal/Task/plan authority.
