...
- The Goal domain owns `GoalExecutionContract`: `goalId`, `version`, `title`, `description`, `constraints` and `successCriteria`. `createGoalExecutionContract` copies the list fields into an immutable snapshot.
...
- Management planning requires the complete contract because its low-level validation/planning surface can operate before a Goal is registered. When the Goal already exists, the composition root compares the submitted snapshot with the authoritative current Goal and rejects any difference before calling the planner.
