...
const files = [...new Set(tracked)]
  .filter((path) => {
    ...
    if (/\.(zip|tar|gz|log|tsbuildinfo|pem|key)$/u.test(path)) return false;
...
const directory = resolve(root, 'artifacts/development');
...
try {
  ...
  const tar = spawnSync(
    'tar',
    ['--null', '-czf', archive, '-T', '-', '-C', metadataDirectory, 'sdar-deployment-source.json'],
