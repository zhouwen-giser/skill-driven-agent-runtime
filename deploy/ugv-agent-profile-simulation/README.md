...
## Existing-stack debug service (current operator configuration)

Run from the SDAR checkout containing the code you want to serve. The existing SMPP services,
Runtime/Control PostgreSQL and Redis must already be running; this command reuses their databases,
registrations and private state rather than recreating them:

```bash
pnpm ugv:debug start       # start SDAR + Control API + worker; final physical side-effect mode YES
pnpm ugv:debug status      # inspect the owned processes and active mode
pnpm ugv:debug restart     # reload all three source processes; final mode YES
pnpm ugv:debug restart NO  # reload with the physical side-effect gate closed
pnpm ugv:debug stop        # stop only these three host processes; keep Docker/data/evidence
```
...
| A2A / Management / Control                  | loopback ports `10999` / `10998` / `10091`                                                      |
...
| Provider northbound                         | Runtime MCP `127.0.0.1:19131/mcp`; PMS `127.0.0.1:18092`                                        |
| Device southbound                           | Adapter-owned `192.168.2.63:19000/mcp` and MQTT `192.168.2.63:1883`                             |
| A2A identity                                | `trusted_intranet`, anonymous initial admission; control/management service tokens stay private |
...
restart does not rerun bootstrap or rewrite Capability/Exposure versions. Missing registrations
still need the formal initial bootstrap rather than being synthesized by this launcher.
