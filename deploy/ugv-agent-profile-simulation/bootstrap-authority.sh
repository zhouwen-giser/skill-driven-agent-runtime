...
uap_record_failure() {
  ...
  node "$uap_repo_root/scripts/ugv-agent-profile-simulation/record-attempt.mjs" \
    bootstrap "$uap_stage" failed "$uap_exit" bootstrap-authority >/dev/null 2>&1 || true
}
...
uap_authority_report="$uap_authority_root/bootstrap-report.json"
uap_authority_log="$(mktemp "$UAP_STATE_ROOT/logs/authority-bootstrap-XXXXXXXX.jsonl")"
...
UGV_SIMULATION_RUN_ID="$(uap_simulation_run_id)" \
...
SDAR_UAP_AUTHORITY_REPORT_FILE="$uap_authority_report" \
  "$uap_repo_root/node_modules/.bin/tsx" \
    ...
    bootstrap >"$uap_authority_log" 2>&1
uap_driver_exit="$?"
set -e
chmod 0600 "$uap_authority_log"
...
node "$uap_repo_root/scripts/ugv-agent-profile-simulation/project-authority-report.mjs" \
  bootstrap "$uap_authority_report" "$UAP_REPORT_ROOT/authority-bootstrap.redacted.json"
...
trap - ERR
node "$uap_repo_root/scripts/ugv-agent-profile-simulation/record-attempt.mjs" \
  bootstrap complete passed 0 bootstrap-authority
