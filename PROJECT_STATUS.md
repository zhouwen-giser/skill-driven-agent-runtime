...
EP-03 Workflow budget update (2026-07-12): approximately 72%. System defaults and current enabled Skill overrides resolve to an immutable per-instance budget; composed Skills use the most restrictive effective limits. LangGraph enforces duration, LLM/MCP counts and configured accounted cost before external calls, including parallel branches, and PostgreSQL stores selected Skill versions, limits, usage and termination. Real e2e proves a Skill `maxMcpCalls: 0` causes zero Tool invocations. `maxReplans` enforcement awaits the FR-WF-008 outer controller, so FR-WF-009 remains developing.
