...
- Capability needed: legacy remote MCP Tool discovery/invocation and the official legacy loopback Server fixture. ADR-090 supersedes its proposed use as the Tasks extension client.
...
- Maintenance and upgrade plan: retain the exact v1 package during the staged adapter migration only for legacy compatibility and loopback Server test support. ADR-090 adds an exact official v2 beta Client for production extension-era negotiation. Removal or upgrade requires contract tests and ADR review.
...
- Decision and ADR: accepted by ADR-002 behind the MCP adapter; ADR-090 supersedes the client-version portion of ADR-085 after the compatibility Spike proved that a `2025-11-25` connection cannot enable the extension. No Resources, Prompts or stdio are added to runtime scope.
