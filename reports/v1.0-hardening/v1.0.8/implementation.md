...
The bug-fixed boundary uses `snapshotGoalExecutionContract` before asynchronous work, so runtime behavior enforces the TypeScript readonly promise. Registered management selection/planning requires the exact active Goal, Goal Patch requires an exact source snapshot, and admin revision copies its source evidence.
