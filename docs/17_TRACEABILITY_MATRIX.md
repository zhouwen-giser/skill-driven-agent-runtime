...
| FR-SKL-002   | EP-02 | unit+integration     | 开发中 | `packages/application/src/skill-authoring.ts`; `packages/application/src/ports.ts`; `packages/application/src/skill-registry.ts`; `packages/management-api/src/http-endpoint.ts` | `packages/application/test/skill-authoring.unit.test.ts`; `packages/a2a-adapter/test/task-service-endpoint.e2e.test.ts` | `pnpm test:unit` + `pnpm test:e2e`: 结构化模型候选经 Zod/Ajv 校验后才持久化并投影 Agent Card；当前 ModelProvider 为注入的模拟实现，生产 Provider 待 EP-03 |
| FR-SKL-003   | EP-02 | unit+integration     | 开发中 | `packages/application/src/skill-authoring.ts`; `packages/management-api/src/http-endpoint.ts` | `packages/application/test/skill-authoring.unit.test.ts`; `packages/management-api/test/http-endpoint.contract.test.ts` | 最多两次生成；歧义描述、非法或不明确 Schema 均失败且零持久化，无宽松兜底；生产模型失败链路待 EP-03 实配验证 |
...
| FR-SKL-012   | EP-02 | unit+integration     | 开发中 | `packages/domain/src/skill-selection.ts`; `packages/application/src/skill-selection.ts`; `packages/application/src/skill-semantic-retriever.ts`; `packages/persistence-postgres/src/repositories.ts`; `infra/postgres/migrations/0011_skill_selection.up.sql`; `infra/postgres/migrations/0013_skill_embedding.up.sql` | `packages/application/test/skill-selection.unit.test.ts`; `packages/application/test/skill-semantic-retriever.unit.test.ts`; `packages/persistence-postgres/test/repositories.integration.test.ts`; `packages/a2a-adapter/test/task-service-endpoint.e2e.test.ts` | `pnpm test:unit` + `pnpm test:integration` + `pnpm test:e2e`: 真实 pgvector 语义分数与成功率、耗时、成本、失败、稳定性快照交给独立 decider 并持久化选择理由；生产模型仍待 EP-03 |
...
| FR-LLM-005   | EP-03 | unit+integration     | 开发中 | `packages/application/src/ports.ts`; `packages/application/src/skill-selection.ts`; `packages/application/src/skill-semantic-retriever.ts`; `packages/persistence-postgres/src/repositories.ts`; `adr/ADR-014-skill-selection-llm-decision-boundary.md`; `adr/ADR-017-pgvector-skill-retrieval-projection.md` | `packages/application/test/skill-selection.unit.test.ts`; `packages/persistence-postgres/test/repositories.integration.test.ts`; `packages/a2a-adapter/test/task-service-endpoint.e2e.test.ts` | 真实 pgvector 仅产生候选语义分数，最终选择只能由独立 Decider 输出并持久化；经验/长期记忆上下文、生产 Provider 与调用审计待 EP-03/05 |
