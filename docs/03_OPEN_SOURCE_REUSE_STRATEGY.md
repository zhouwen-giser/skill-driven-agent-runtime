...
- MCP Streamable HTTP client 的连接、发现、调用、取消、超时和错误。v1.1 Spike 由 ADR-090 固化：v1 不能启用 extension；v2 beta.4 自动协商可用，但 SEP-2663 的 method/result/Header 缺口必须由 Adapter 内临时 bridge 和精确 wire 合约覆盖。
