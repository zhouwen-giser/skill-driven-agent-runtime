...
if(lock.consumedContracts.WorkflowPattern?.version!=='1.2'||manifest.nextPackage!=='P04R') throw new Error('P04R handoff');
for(const name of ['FusedPattern','GeneralizedPattern','CandidateStaticValidationResult']) if(lock.producedContracts[name]?.version!=='1.2') throw new Error(`v1.2 contract ${name}`);
