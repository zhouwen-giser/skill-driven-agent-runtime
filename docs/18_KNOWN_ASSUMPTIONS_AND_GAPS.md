...
- 2026-07-11（2026-07-13 更新）：Docker Desktop 现可返回 Engine 29.6.1/version 与容器清单，但变更型操作仍异常：仓库 `smoke:infra` 的 `compose up --wait` 在 180 秒后超时，后续 `compose up` 与直接 `docker start` 也挂起；SDAR PostgreSQL/Redis 容器保持 exited，54329/56379 端口关闭。不得以只读 Engine/version 或静态 Compose 代替真实 PostgreSQL/pgvector/Redis 验证；重启整个 Docker Desktop 会影响仓库范围外容器，未擅自执行。
