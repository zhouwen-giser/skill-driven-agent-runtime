...
| v1.0.11 | MCP Tool execution semantics          | ADR-082; MCP-declared → retained Admin override → unknown; immutable Planner/Invocation snapshots; atomic override/audit | domain/application/MCP adapter/PostgreSQL/management/Console vertical path | `0063_mcp_tool_execution_semantics` | unit+contract+integration+migration+real A2A/Model/MCP E2E | `2efbef6` / `v1.0.11` | this bug-fixed commit / `v1.0.11-bug-fixed` pending | feature: 281 unit, 62 contract, 59 integration, 46 E2E; bug-fixed full `pnpm verify`: 283 unit, 63 contract, 59 integration, 46 E2E, 182-source architecture, 105-operation OpenAPI, build+migrations+smoke passed in 85,191 ms | no MCP Task binding/polling/device authority | bug-fixed gate passed; publication pending |
