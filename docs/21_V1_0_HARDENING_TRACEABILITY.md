...
| v1.0.9  | Skill Graph composition planning      | ADR-080; bounded non-alternative graph snapshot; model subset decision; persisted authorization | domain/application/runtime/PostgreSQL/management vertical path | `0062_skill_composition_context` | unit+contract+integration+migration+real A2A/Model/MCP E2E | this feature commit / `v1.0.9` | pending | feature: format/lint/typecheck, 270 unit, 59 contract, 58 integration, 46 E2E, 181-source architecture, 104-operation OpenAPI, build+migrations passed; full `pnpm verify` required at bug-fixed | traversal depth 8 / related Skills 32; historical pre-0062 plans remain readable but do not grant new planning authority | feature gate passed |
