...
| v1.0.11 | MCP Tool execution semantics          | ADR-082; MCP-declared → retained Admin override → unknown; Planner/Invocation snapshots | domain/application/MCP adapter/PostgreSQL/management/Console vertical path | `0063_mcp_tool_execution_semantics` | unit+contract+integration+migration+real A2A/Model/MCP E2E | this feature commit / `v1.0.11` | pending | full `pnpm verify`: 281 unit, 62 contract, 59 integration, 46 E2E, 182-source architecture, 105-operation OpenAPI, build+migrations+smoke passed in 86,700 ms | no MCP Task binding/polling/device authority | feature gate passed |
