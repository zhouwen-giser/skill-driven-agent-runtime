...
| v1.0.8  | complete Goal execution contract      | ADR-079; one domain snapshot; exact content equality at authority boundaries | domain/application/runtime/PostgreSQL/management vertical path | `0061_goal_execution_contract` | unit+contract+integration+migration+real A2A/Model/MCP E2E | this feature commit / `v1.0.8` | pending | feature: format/lint/typecheck, 259 unit, 59 contract, 57 integration, 46 E2E, 178-source architecture, 104-operation OpenAPI, build+migrations passed | unrecoverable legacy rows use an explicit compatibility snapshot and cannot authorize new work | feature gate passed; bug-fixed pending |
