...
| v1.0.2  | real `skill_call` child workflows     | ADR-073; normal planner/validator/executor; child-scoped Tool policy; bounded ancestry | `packages/application/src/skill-call-workflow.ts`; `packages/application/src/workflow-execution.ts`; `apps/server/src/runtime.ts` | none at feature | unit+integration+real MCP E2E | this feature commit / `v1.0.2` | pending | format, lint, typecheck, architecture, 204 unit, 57 contract, 36 integration, 41 E2E passed | nested confirmation finalized in v1.0.5; repeated-node linkage pending bug-fixed | feature gate passed |
