...
| v1.0.2  | real `skill_call` child workflows     | ADR-073; normal planner/validator/executor; child-scoped Tool policy; bounded ancestry/output | application/runtime plus append-only relation repository | `0054_skill_call_history` in bug-fixed | unit+integration+migration+real MCP E2E | `0e3122c` / `v1.0.2` | this bug-fixed commit / `v1.0.2-bug-fixed` | feature: 204 unit, 57 contract, 36 integration, 41 E2E; bug-fixed: format/lint/typecheck/architecture, 206 unit, 57 contract, 37 integration, 41 E2E, build+migrations passed | nested confirmation finalized in v1.0.5; output max 64,000 chars | bug-fixed gate passed |
