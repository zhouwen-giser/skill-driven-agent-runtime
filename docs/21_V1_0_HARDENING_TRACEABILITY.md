...
| v1.0.5  | nested Skill confirmation             | ADR-076; one conservative transitive evaluator; independent child checkpoint | domain/application/LangGraph/Task/runtime/PostgreSQL vertical path | `0057_nested_skill_confirmation` | unit+real migration+real A2A/MCP E2E | this feature commit / `v1.0.5` | pending | feature: format/lint/typecheck, 229 unit, 58 contract, 43 integration, 43 E2E, architecture/build/migrations passed | paused checkpoints remain process-local per V1 non-recovery rule | feature gate passed |
