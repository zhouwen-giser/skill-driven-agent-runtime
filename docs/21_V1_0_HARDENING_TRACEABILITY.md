...
| v1.0.6  | atomic authoritative terminal outcome | ADR-077; one PostgreSQL authority transaction; isolated enhancements | domain/application/runtime/PostgreSQL vertical path | `0058_runtime_terminal_outcome` | unit+fault-injection integration+migration+real A2A E2E | this feature commit / `v1.0.6` | pending / `v1.0.6-bug-fixed` | feature: 242 unit, 58 contract, 52 integration, 44 E2E, 175-source architecture, build+migrations passed; full `pnpm verify` required at bug-fixed | Goal-wide multi-Task cancellation retains its existing cascade boundary | feature gate passed |
