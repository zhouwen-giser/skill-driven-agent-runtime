...
| v1.0.5  | nested Skill confirmation             | ADR-076; one conservative transitive evaluator; exact serialized child decisions | domain/application/LangGraph/Task/runtime/PostgreSQL vertical path | `0057_nested_skill_confirmation` | unit+real migration+real A2A/MCP E2E | `6decc5d` / `v1.0.5` | this bug-fixed commit / `v1.0.5-bug-fixed` | feature: 229 unit, 58 contract, 43 integration, 43 E2E; bug-fixed full gate: 238 unit, 58 contract, 43 integration, 43 E2E, architecture/build/migrations/smoke passed | paused checkpoints remain process-local per V1 non-recovery rule | bug-fixed gate passed |
