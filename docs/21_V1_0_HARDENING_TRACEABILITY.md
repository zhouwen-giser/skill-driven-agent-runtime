...
| v1.0.12 | Memory durability/embedding hardening | ADR-083; strict seven-field refinement; durable-only admission; generic positive vectors with exact provider/dimension matching; post-commit warning query | domain/application/PostgreSQL/management/Console vertical path | `0064_memory_production_hardening` | unit+contract+integration+migration+real A2A/Model/MCP E2E | this feature commit / `v1.0.12` pending | pending / `v1.0.12-bug-fixed` | feature full `pnpm verify`: 284 unit, 64 contract, 60 integration, 46 E2E, 182-source architecture, 106-operation OpenAPI, build+migrations+smoke passed in 86,193 ms | legacy rows are unknown/excluded; rollback rejects non-3D rows; MCP remains live-state authority | feature gate passed; publication pending |
