  ...
  it('recovers after the parent continuation commits but its remote-control callback crashes', async () => {
    ...
    const skill = createSkillVersion({
      ...
      outcomeSpecification: {
        ...
        skillId: 'remote-child-skill',
        skillVersion: 1,
        specificationHash: `sha256:${createHash('sha256').update('remote-child-skill:1').digest('hex')}`,
        effects: ['effect.child_completed'],
        evidence: ['evidence.child_result'],
        artifacts: [],
        taskGoalPolicy: {},
        confidencePolicy: {},
        sideEffectPolicy: {},
...
function remoteAdmission(
  ...
  return {
    ...
    authoritySnapshot: {
      ...
      runtime: {
        serverId: authority.serverId,
        endpoint: `https://${authority.serverId}.test/mcp`,
