...
CREATE TABLE artifact_lineage (
  ...
  artifact_id text NOT NULL,
  artifact_version integer NOT NULL CHECK (artifact_version >= 1),
  source_episode_refs jsonb NOT NULL CHECK (jsonb_typeof(source_episode_refs) = 'array'),
  source_knowledge_refs jsonb NOT NULL CHECK (jsonb_typeof(source_knowledge_refs) = 'array'),
  source_correction_refs jsonb NOT NULL CHECK (jsonb_typeof(source_correction_refs) = 'array'),
  source_pattern_refs jsonb NOT NULL CHECK (jsonb_typeof(source_pattern_refs) = 'array'),
  generation_methods jsonb NOT NULL CHECK (jsonb_typeof(generation_methods) = 'array'),
...
CREATE TABLE artifact_validation_run (
  ...
  counterexample_refs jsonb NOT NULL CHECK (jsonb_typeof(counterexample_refs) = 'array'),
...
CREATE TABLE artifact_feedback (
  ...
  artifact_execution_id text NOT NULL REFERENCES artifact_execution(artifact_execution_id),
...
CREATE TABLE experience_trace (
  ...
  task_type_refs jsonb NOT NULL CHECK (jsonb_typeof(task_type_refs) = 'array'),
...
CREATE TABLE pattern_candidate (
  ...
  support_refs jsonb NOT NULL CHECK (jsonb_typeof(support_refs) = 'array'),
  contradiction_refs jsonb NOT NULL CHECK (jsonb_typeof(contradiction_refs) = 'array'),
